...
const Shifts = () => {
    ...
    const HandleUpdate = async (shift) => {
        ...
        if (response.ok) {
            ...
            const updatedIndex = shiftListCopy.findIndex(elem => elem.id === json.id);
    ...
    return (
        <div className="container align-items-center">
            <h1 className="p-2 m-2">Shifts</h1>
            <div className="container align-items-center">
                {shiftList && <div className="employeeTypes">
                    <table className="table table-light table-bordered table-striped table-responsive">
                        ...
                        <tbody >
                            {shiftList.map(shift => <ShiftRow key={(shift.id)} shift={shift} DeleteShift={DeleteShift} loading={loading} HandleUpdate={HandleUpdate} />)}
                            {newRow && <ShiftNewRow shift={newShift} loading={loading} HandlePost={HandlePost} setNewRow={setNewRow} />}
                        </tbody>
                    </table>
                </div>}
                ...
                {!loading && <button className="btn btn-primary w-auto" onClick={() => setNewRow(true)}>Add a new shift</button>}
            </div >
        </div>
    );
}
